      ...
      <div class="col-md-12 mt-3">
        ...
        {{ csrf_field() }}
          <h3>{{ $shop->product_name }}</h3>
            ...
            @if($shop->product_price_type == 'single_price')
                @if($shop->product_offer_price != 0)
                    <span class="fs32 display_result">{{ $allsettings->site_currency_symbol }}{{ $shop->product_offer_price }}
                    </span>
                @else
                    <span class="fs32 display_result">{{ $allsettings->site_currency_symbol }}{{ $shop->product_price }}</span>
                @endif
            @else
                <input type="hidden" name="price" class="bulk_quantity">
                <span class="fs32 display_result">
                    {{ $allsettings->site_currency_symbol }}{{ number_format(($shop->product_qty_1*$shop->product_price_1),2) }}
                </span>
                ...
                <table class="table table-striped">
                    <tbody>
                        <tr>
                            <td><b>Qty</b></td>
                            <td class="text-center">{{ $shop->product_qty_1 }}</td>
                            <td class="text-center">{{ $shop->product_qty_2 }}</td>
                            <td class="text-center">{{ $shop->product_qty_3 }}</td>
                            <td class="text-center">{{ $shop->product_qty_4 }}</td>
                            <td class="text-center">{{ $shop->product_qty_5 }}</td>
                            <td class="text-center">{{ $shop->product_qty_6 }}</td>
                        </tr>
                        <tr>
                            <td><b>Price({{ $allsettings->site_currency_symbol }})</b></td>
                            <td class="text-center">{{ $shop->product_price_1 }}</td>
                            <td class="text-center">{{ $shop->product_price_2 }}</td>
                            <td class="text-center">{{ $shop->product_price_3 }}</td>
                            <td class="text-center">{{ $shop->product_price_4 }}</td>
                            <td class="text-center">{{ $shop->product_price_5 }}</td>
                            <td class="text-center">{{ $shop->product_price_6 }}</td>
                        </tr>
                    </tbody>
                </table>
            @endif
            ...
            @endif
                ...
                --}}
                                         ...
                                         @if($shop->product_type != 'digital')
                                           ...
                                           @else
                                                <span>
                                                  <label>{{ Helper::translation(2145,$translate) }}</label>
                                                  <input type="number" name="qty" class="form-control qty_bulk" id="qty_bulk" min="{{$shop->product_qty_1}}" data-bvalidator="required,min[{{$shop->product_qty_1}}]" value="{{$shop->product_qty_1}}">
                                               </span>
                                           @endif
              ...
              <button type="button" class="btn btn-success float-left ml-1 sellerMessage" data="{{ $seller->id }}" data-toggle="modal" >Message Seller</button>
              @endif
            ...
            <!--  <div class="row">
             ...
             <div class="footer-box-info mt-3 mb-3 pt-3 pb-3 clearfix">
                <button type="button" class="btn btn-success float-left ml-1 sellerMessage mgb" data-toggle="modal">Message Seller</button>

                <button type="button" class="btn btn-dark mb-3 form-control odr" data-toggle="modal" data-target="#customizeOrderModal">Request a customize order</button>  
                <br />
          ...
          <div class="container mt-2 mb-2">
		    <div class="row">
				<div class="col-md-2">
                        ...
                        <img  src="{{ url('/') }}/public/storage/users/{{ $rating->user_photo }}" alt="{{ $rating->username }}" class="media-object" style="height:100px !important">
                        @else
				...
				<div class="col-md-2"><div class="date"> <span class="text-muted">2 May</span> </div></div>
		</div>
        </div>
        @endforeach                 
          </div>
    ...
    <div class="col-md-4">
        ...
        <div class="d-flex flex-row align-items-center"> <img src="https://i.imgur.com/N2fYgvD.png" class="rounded-circle store-image">
                        <div class="d-flex flex-column ml-1 comment-profile">
                            <div class="comment-ratings"> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i> </div> <span class="username" style="font-weight: bold;"> Rare Boutique</span> <small class="followers">25K Followers</small>
                        </div>
                    </div>
        </div>
        ...
        <h4 class="black mb-2 pb-2 text-center">{{ Helper::translation(2152,$translate) }}</h4>
                            ...
                            @php $z = 1; @endphp
                              ...
                              <div class="col-lg-3 col-md-6 col-sm-6 col-xs-6 mb-2 pb-2">
                               ...
                               <i class="fa fa-eye"></i>
                               <p class="quick_view">{{ Helper::translation(2060,$translate) }}<br/>{{ Helper::translation(2061,$translate) }}</p>
                               </a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="inbox_id" value="new">
         <textarea name="message" class="form-control" id="" cols="45" rows="3" placeholder="{{ Helper::translation(2928,$translate) }}" data-bvalidator="required"></textarea><br>
...
<script type="text/javascript">var URL = {!! json_encode(url('/')) !!}</script>
...
$(document).ready(function() { 
    ...
    $('#qty_bulk').change(function(){
      ...
      var product_id = '{{ $shop->product_id }}';

      $.ajax({
            url:"{{ url('product_price/') }}"+"/"+quantity+"/"+product_id,
            method:'get',
            data:{},
            success:function(data){ 
                // console.log(data);
                $('.display_result').text("{{ $allsettings->site_currency_symbol }}" + data);
              },
        });
...
@include('component.request_customize_order')
